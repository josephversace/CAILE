...
namespace IIM.Infrastructure.Models
{
    ...
    public class ModelConfigurationService : IModelConfigurationService
    {
        ...
        private readonly IOptions<ModelConfigurationConfiguration> _config;
        private readonly ConcurrentDictionary<string, ModelConfiguration> _metadata = new();
        private readonly SemaphoreSlim _loadLock = new(1, 1);
        private bool _isLoaded = false;

        public ModelConfigurationService(
            ILogger<ModelConfigurationService> logger,
            IOptions<ModelConfigurationConfiguration> config)
        {
        ...
        /// </summary>
        public async Task<ModelConfiguration> GetMetadataAsync(string modelId, CancellationToken ct = default)
        {
        ...
        /// </summary>
        public Task RegisterMetadataAsync(ModelConfiguration metadata, CancellationToken ct = default)
        {
            if (metadata == null)
        ...
        public async Task<List<ModelConfiguration>> GetAllMetadataAsync(CancellationToken ct = default)
        {
            ...
            return new List<ModelConfiguration>(_metadata.Values);
        }
        ...
        private async Task LoadFromFileAsync(string filePath, CancellationToken ct)
        {
            try
            {
                if (File.Exists(filePath))
                {
                    ...
                    var models = JsonSerializer.Deserialize<List<ModelConfiguration>>(json);
        ...
        {
            // LLM Models
            RegisterDefaultIfMissing(new ModelConfiguration
            {
        ...
        /// </summary>
        private void RegisterDefaultIfMissing(ModelConfiguration metadata)
        {
        ...
        /// </summary>
        private ModelConfiguration CreateDefaultMetadata(string modelId)
        {
            // Infer properties from model ID
            var metadata = new ModelConfiguration
            {
                ModelId = modelId,
        ...
        Task<Shared.Models.ModelConfiguration> IModelConfigurationService.GetMetadataAsync(string modelId, CancellationToken ct)
        {
        ...
        Task<List<Shared.Models.ModelConfiguration>> IModelConfigurationService.GetAllMetadataAsync(CancellationToken ct)
        {
            throw new NotImplementedException();
        }
    }
